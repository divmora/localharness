...
func TestEnrichUserMessage_Full(t *testing.T) {
	...
	brainDir := t.TempDir()
	_ = os.WriteFile(filepath.Join(brainDir, "plan.md"), []byte("plan"), 0644)
	_ = os.WriteFile(filepath.Join(brainDir, "task.md"), []byte("task"), 0644)
	_ = os.WriteFile(filepath.Join(brainDir, ".hidden"), []byte("skip"), 0644)
