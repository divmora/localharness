...
func TestLoadAgentsRules_Found(t *testing.T) {
	dir := t.TempDir()
	_ = os.WriteFile(filepath.Join(dir, "AGENTS.md"), []byte("# Rules\nKeep docs in sync.\n"), 0644)
...
func TestLoadAgentsRules_AlternateFilename(t *testing.T) {
	dir := t.TempDir()
	_ = os.WriteFile(filepath.Join(dir, ".agents.md"), []byte("alternate rules"), 0644)
...
func TestLoadAgentsRules_PriorityOrder(t *testing.T) {
	dir := t.TempDir()
	_ = os.WriteFile(filepath.Join(dir, "AGENTS.md"), []byte("primary rules"), 0644)
	_ = os.WriteFile(filepath.Join(dir, ".agents.md"), []byte("alternate rules"), 0644)
...
func TestLoadAgentsRules_MultipleWorkspaces(t *testing.T) {
	dir1 := t.TempDir()
	dir2 := t.TempDir()
	_ = os.WriteFile(filepath.Join(dir1, "AGENTS.md"), []byte("rules from ws1"), 0644)
	_ = os.WriteFile(filepath.Join(dir2, "AGENTS.md"), []byte("rules from ws2"), 0644)
...
func TestLoadAgentsRules_EmptyFile(t *testing.T) {
	dir := t.TempDir()
	_ = os.WriteFile(filepath.Join(dir, "AGENTS.md"), []byte("  \n  \n"), 0644) // Whitespace only
