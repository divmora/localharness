...
func TestDiscoverSkills(t *testing.T) {
	...
	_ = os.MkdirAll(scannerDir, 0755)
	_ = os.WriteFile(filepath.Join(scannerDir, "SKILL.md"), []byte(`---
name: scanner
...
`), 0644)
	...
	_ = os.MkdirAll(linterDir, 0755)
	_ = os.WriteFile(filepath.Join(linterDir, "SKILL.md"), []byte(`---
name: linter
...
`), 0644)
	...
	_ = os.MkdirAll(filepath.Join(dir, "empty-dir"), 0755)

	// file (not a directory)
	_ = os.WriteFile(filepath.Join(dir, "not-a-dir.txt"), []byte("hello"), 0644)
...
func TestDiscoverPlugins(t *testing.T) {
	...
	_ = os.MkdirAll(filepath.Join(secDir, "skills", "scan"), 0755)
	_ = os.WriteFile(filepath.Join(secDir, "plugin.json"), []byte(`{"name":"securecoder","description":"Security tools"}`), 0644)
	_ = os.WriteFile(filepath.Join(secDir, "skills", "scan", "SKILL.md"), []byte(`---
name: scan
...
`), 0644)
	...
	_ = os.WriteFile(filepath.Join(disabledDir, "plugin.json"), []byte(`{"name":"old","disabled":true}`), 0644)

	// broken plugin (invalid JSON)
	brokenDir := filepath.Join(dir, "broken-plugin")
	_ = os.MkdirAll(brokenDir, 0755)
	_ = os.WriteFile(filepath.Join(brokenDir, "plugin.json"), []byte(`{invalid`), 0644)
...
func TestDiscoverAll(t *testing.T) {
	...
	_ = os.MkdirAll(globalSkillDir, 0755)
	_ = os.WriteFile(filepath.Join(globalSkillDir, "SKILL.md"), []byte(`---
name: global-skill
...
`), 0644)
	...
	_ = os.MkdirAll(globalPluginDir, 0755)
	_ = os.WriteFile(filepath.Join(globalPluginDir, "plugin.json"), []byte(`{"name":"global-plugin","description":"Global plugin"}`), 0644)

	// Workspace skill (same name as global — should override)
	wsSkillDir := filepath.Join(workspaceDir, ".agents", "skills", "global-skill")
	_ = os.MkdirAll(wsSkillDir, 0755)
	_ = os.WriteFile(filepath.Join(wsSkillDir, "SKILL.md"), []byte(`---
name: global-skill
...
`), 0644)
	...
	_ = os.MkdirAll(wsOnlyDir, 0755)
	_ = os.WriteFile(filepath.Join(wsOnlyDir, "SKILL.md"), []byte(`---
name: ws-only
