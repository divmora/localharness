...
func TestInterruptResume_ManualInterrupt(t *testing.T) {
	...
	// Simulate a turn
	_, _ = ir.PreTurn(context.Background(), &TurnRequest{
		Prompt:   "Deploy to production",
		Metadata: make(map[string]any),
	})

	// Simulate some steps
	_, _ = ir.ProcessStep(context.Background(), &StepEvent{TextDelta: "Checking ", Metadata: make(map[string]any)})
	_, _ = ir.ProcessStep(context.Background(), &StepEvent{TextDelta: "status...", Metadata: make(map[string]any)})
...
func TestInterruptResume_TimeoutDetection(t *testing.T) {
	...
	_, _ = ir.PostTurn(context.Background(), &TurnResponse{
		Text:        "partial result",
		TotalTokens: 500,
		StepCount:   3,
		Metadata:    make(map[string]any),
...
func TestInterruptResume_MultipleInterrupts(t *testing.T) {
	...
	// Turn 1
	_, _ = ir.PreTurn(context.Background(), &TurnRequest{Prompt: "task 1", Metadata: make(map[string]any)})
	cp1 := ir.Interrupt(InterruptUser, "first")

	// Turn 2
	_, _ = ir.PreTurn(context.Background(), &TurnRequest{Prompt: "task 2", Metadata: make(map[string]any)})
