...
- **Maintain the living product roadmap.** When identifying future enhancements or tech debt, record them in [ROADMAP.md](ROADMAP.md). When an item is implemented and verified, prune it from [ROADMAP.md](ROADMAP.md).
- **Enforce workspace boundary safety.** Always validate filesystem access against configured workspace boundaries.
...
- **Verification**: `make fmt && make lint && make test && make build`
...
| `~/.divmora/localharness/knowledge/<project-uuid>/codegraph.db` | Repository AST code graph (SQLite-indexed, WAL mode) |
