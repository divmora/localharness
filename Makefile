.PHONY: proto build test clean lint fmt deps test-client build-lhctl all cross-compile
...
# Lint proto and Go source files
lint:
	@echo "==> Linting..."
	@if command -v buf >/dev/null 2>&1; then buf lint; fi
	@go vet ./...
	@if command -v golangci-lint >/dev/null 2>&1; then golangci-lint run; elif [ -x "$$(go env GOPATH)/bin/golangci-lint" ]; then "$$(go env GOPATH)/bin/golangci-lint" run; fi
